...
(function ($) {
  $.fn.tableExport = function (options) {
    var defaults = {
      ...
      outputMode:         'window',      // 'file', 'string', 'base64' or 'window' (experimental)
      pdfmake: {
        enabled:           true,       // true: use pdfmake instead of jspdf and jspdf-autotable (experimental)
        docDefinition: {
          pageOrientation: 'portrait',  // 'portrait' or 'landscape'
          defaultStyle: {
            font:          '微软雅黑'     // Default is 'Roboto', for arabic font set this option to 'Mirza' and include mirza_fonts.js
          }
        },
        fonts: {}
      },
      ...
      theadSelector:       'tr',
      tableName:           'Table',
      type:                'pdf'        // 'csv', 'tsv', 'txt', 'sql', 'json', 'xml', 'excel', 'doc', 'png' or 'pdf'
    };
    ...
    if ( defaults.type === 'csv' || defaults.type === 'tsv' || defaults.type === 'txt' ) {
      ...
      if ( defaults.pdfmake.enabled === true ) {
        ...
        var colcount = CollectPdfmakeData($hrows, 'tr,th', $hrows.length);
        ...
        CollectPdfmakeData($rows, 'tr,td', $hrows.length + $rows.length);
        ...
        pdfMake.createPdf(docDefinition).getBuffer(function (buffer) {
          ...
          try {
            var blob = new Blob([buffer], {type: "application/adobe-pdf"});
    ...
    */
    ...
    function jsPdfOutput (doc, hasimages) {
      ...
      try {
        ...
        downloadFile(defaults.fileName + '.pdf',
          'data:application/adobe-pdf' + (hasimages ? '' : ';base64') + ',',
      ...
      if ( filename !== false && window.navigator.msSaveOrOpenBlob ) {
        ...
        if ( DownloadLink ) {
          ...
          if ( typeof data === 'object' ) {
			...
			window.URL = window.URL || window.webkitURL;
            blobUrl = window.URL.createObjectURL(new Blob(binaryData, {type: "application/adobe-pdf"}));
