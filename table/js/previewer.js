...
(function (factory) {
  ...
  Previewer.prototype = {
    ...
    show: function () {
      ...
      var ifr = $previewer
      .find('.previewer-sidebar').click(function(e){previewerObj.click(e)})
      ...
      if(options.method == "GET") {
         ifr.attr('src', addTimestamp(location.href));
      } else {
         $("#reportQuery-form").submit();
      }
      previewerObj.previewer = $previewer;
    },
      ...
      if (data.dismiss) {
        ...
        if (data && data.preview) {
          ...
          if(options.method == "GET") {
             ...
             options.type = data.preview;
             var $previewer = this.previewer;
             $previewer.find('.previewer-nav a').removeClass('active');
             $previewer.find('[data-preview="' + options.type + '"]').addClass('active');
             $previewer.find('iframe').width(options[options.type]);
             $("#reportQuery-form").submit();
          }
        }
      }
  ...
  Previewer.TEMPLATE = (
    '<div class="previewer-container">' +
      '<div class="previewer-sidebar">' +
        '<button data-dismiss="previewer" title="Close" disabled="disabled"></button>' +
      ...
      '<div class="previewer-mainbody">' +
        '<iframe scrolling="no" name="previewerIframe"></iframe>' +
      '</div>' +
    '</div>'
  );
