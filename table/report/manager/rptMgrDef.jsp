...
<div class="main" style="padding-top:60px;">
	   <div class="header-top navbar fixed-top" style="margin-left:60px;padding:16px;">
           <span style="padding-left:60px;font-size:14px;"><b>报表作品</b></span>
     </div>
	 <div style="position:fixed;top:0;float:left;width:60px;height:100%;background-color: #3b3f51;z-index:20;padding-top:20px;">
	       <ul class="left-tool-menu">
	        <li>
	          <a href="${path}/rptMgr/def/add_rpt.htm"><i class="icon iconfont icon-baobiaosheji16x16 fa-lg" style="font-size:24px"></i><span class="icon-name">报表设计</span></a>
	        </li>
	        <li>
	          <a href="${path}/source/index.htm?type=def"><i class="icon iconfont icon--shujuyuan fa-lg" style="font-size:28px"></i><span class="icon-name">数据源</span></a>
	        </li>
	        <li class="left-tool-menu-active">
	          <a href="${path}/rptMgr/def/index.htm"><i class="icon iconfont icon-baobiao fa-lg" style="font-size:24px"></i><span class="icon-name">报表</span></a>
	        </li>
	       </ul>
      </div>
	...
	<script type="text/javascript">
	   ...
	   function zTreeOnClick(event, treeId, treeNode) {
	      ...
	      if(!selectedTreeNode.isParent) {
	         ...
	         $("#rptMgr-form").attr("action", '${path}/' + selectedTreeNode.key + ".rpt");
	         $("#rptName").empty().html(selectedTreeNode.name);
	         $("#rptUrl").empty().html('${basePath}' + selectedTreeNode.key + ".rpt");
	   ...
	   $(function() {
	       ...
	       $("#icon-add-folder").click(function() {
	          ...
	          layer.open({
				  ...
				  yes:function(index) {
				     ...
				     $.ajax({
						   type: "POST",
						   url: "${path }/rptMgr/def/add_folder.htm",
						   data: jsonData,
						   dataType : "json",
						   success: function(cf){
						     if(!cf) {
	       ...
	       $("#icon-mod-folder").click(function(){
	           ...
	           layer.open({
				  ...
				  yes:function(index) {
				     ...
				     $.ajax({
						   type: "POST",
						   url: "${path }/rptMgr/def/mod_folder.htm",
						   data: jsonData,
						   dataType : "json",
						   success: function(cf){
						     if(!cf) {
	       ...
	       $("#icon-del-folder").click(function() {
		       ...
		       layer.confirm('您确定要删除这个目录及目录中的报表文件吗?', {icon: 3, title:'提示'}, function(index){	
		           ...
		           $.ajax({
					   type: "POST",
					   url: "${path }/rptMgr/def/del_folder.htm",
	       ...
	       $("#icon-move-folder").click(function() {
	            ...
	            $.ajax({
				   type: "POST",
				   url: "${path}/rptMgr/def/find_all_folder.htm",
				   dataType : "json",
				   success: function(moveTree){
	                   ...
	                   layer.open({
						  ...
						  yes:function(index) {
							   ...
							   $.ajax({
								   type: "POST",
								   url: "${path }/rptMgr/def/move_folder.htm",
	       ...
	       $("#btn-add-rpt").click(function() {
	            ...
	            var index = layer.open({
				    ...
				    maxmin:true,
				    fix:false,
					content: "${path}/rptMgr/def/add_rpt.htm?folderId=" + selectedTreeNode.id
				});
	       ...
	       $("#btn-mod-rpt").click(function() {
	            ...
	            document.location.href = "${path}/rptMgr/def/update_rpt.htm?rptId=" + selectedTreeNode.key;
	       });
	       
	       $("#btn-del-rpt").click(function() {
	            if(!selectedTreeNode) {
	               layer.alert('请选择将要删除的报表!', {icon: 0});
	               return false;
	            }
	            if(selectedTreeNode.isParent) {
	               layer.alert('请选择将要删除的报表!', {icon: 0});
	            ...
	            layer.confirm('您确定要删除该报表吗?', {icon: 3, title:'提示'}, function(index){	
	               layer.close(index);
		           $.ajax({
					   type: "POST",
					   url: "${path }/rptMgr/def/del_rpt_tplt.htm",
					   data: {rptId:selectedTreeNode.key},
					   dataType : "json",
					   success: function(suc){
					     if(!suc) {
					        layer.alert('报表删除出现错误!', {icon: 2});
					     } else {
					        treeObject.removeNode(selectedTreeNode);
					        var parentNode = selectedTreeNode.getParentNode();
					        if(parentNode != null && !parentNode.isParent) {
